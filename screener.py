"""
Parallel NSE screener — SMC + ICT strategy.
Stock list pulled LIVE from NSE API (no hardcoding).
Returns only signals that pass ALL hard rules (score 65+/150).
"""
import time
...
from config        import STRONG_SCORE, SCAN_WORKERS, MAX_STOCKS
...
def scan_market() -> list:
    ...
    ist_time = _now_ist()                  # single snapshot for entire scan
    strong   = []

    print(f"\n[SCAN] {len(stocks)} stocks | {SCAN_WORKERS} workers | "
          f"threshold={STRONG_SCORE}/150 | "
          f"KZ={_get_kz_label(ist_time)}")
    start = time.time()
...
def _get_kz_label(ist_time: datetime) -> str:
    from strategy import _get_kill_zone
    name, _ = _get_kill_zone(ist_time.hour, ist_time.minute)
    return name
